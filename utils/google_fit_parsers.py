...
def google_interval_datastream_parser(raw_records, stream_name, stream_info, personicle_user_id):
    ...
    for point in data_points:
        return_message['dataPoints'].append({
            #"timestamp": str(datetime.fromtimestamp(int(point['endTimeNanos'])/10**9)),# point['endTimeNanos'],
            "start_time": str(datetime.fromtimestamp(int(point['startTimeNanos']) / 10 ** 9)),
            "end_time": str(datetime.fromtimestamp(int(point['endTimeNanos']) / 10 ** 9)),
            "value": _datapoint_formatter(point['value'][0], stream_info['ValueType'])
        })
    return return_message
 ...
 "bucket": [
  {
   ...
   "dataset": [
    ...
    sleepstage={1:"Awake (during sleep cycle)", 2:"Sleep",3:"Out-of-bed",
    ...
    for point in data_points:
        return_message['dataPoints'].append({
            ...
            "value": sleepstage.get(point['value'][0]['intval'])
            #"value": _datapoint_formatter(point['value'][0]['intval'], stream_info['ValueType']) #.map(sleepstage) #sleepstage

        })

    #return_message["dataPoints"] = {k: sleepstage.get(v, v) for k, v in return_message["dataPoints"][0].items()}
