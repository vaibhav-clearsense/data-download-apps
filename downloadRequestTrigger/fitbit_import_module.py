...
def fitbit_sleep_import(personicle_user_id, access_token, last_accessed_at, fitbit_user_id):
    sleep_api_endpoint = os.environ['FITBIT_API_ENDPOINT'] + FITBIT_SLEEP_LOG_ENDPOINT.format(user_id=fitbit_user_id)
    ...
    if 'sleep' not in sleep:
        ...
        return False, sleep
    send_response = fitbit_upload.send_records_to_personicle(personicle_user_id, sleep['sleep'], 'sleep')
    LOG.info(send_response)


def fitbit_activity_import(personicle_user_id, access_token, last_accessed_at, fitbit_user_id):
